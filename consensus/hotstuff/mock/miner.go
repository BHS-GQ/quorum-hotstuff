...
type environment struct {
	state     *state.StateDB
	privstate mps.PrivateStateRepository
	header    *types.Header
...
func (m *miner) newWork() {
	parent := types.CopyHeader(m.chain.CurrentHeader())
	num := parent.Number
	...
	if err := types.HotstuffHeaderFillWithValidators(header, nil); err != nil {
	...
	block, err := m.engine.FinalizeAndAssemble(m.chain, m.current.header, s, nil, nil, nil) // receipts are nil
	if err != nil {
	...
	m.pendingMu.Lock()
	task := &task{receipts: nil, state: s, block: block}
...
func (m *miner) makeCurrent(header *types.Header) {
	...
	statedb, privstatedb, _ := m.chain.StateAt(block.Root())
	m.current = &environment{
		header:    header,
		state:     statedb,
		privstate: privstatedb,
	}
}
